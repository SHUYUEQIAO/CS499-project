...
# step 1:get the data
Data_training <- data.table::fread('https://web.stanford.edu/~hastie/ElemStatLearn/datasets/zip.train.gz')
Data_test <- data.table::fread('https://web.stanford.edu/~hastie/ElemStatLearn/datasets/zip.test.gz')

#step 2:data processing
Data_Combined <- rbind(Data_training,Data_test)
m <- nrow(Data_Combined)
n <- ncol(Data_Combined)
Data_No_Scale <- matrix(0,ncol = n, nrow = 1)
for( index in 1:m )
  {
  if( (Data_Combined[index,1] == 1) ||
      (Data_Combined[index,1] == 0))
  {
    Data_No_Scale <- rbind(Data_No_Scale,Data_Combined[index,])
  }
}

#scale the inputs 
Data_No_Scale_X <- Data_No_Scale[,-1]
Data_No_Scale_y <- Data_No_Scale[,1]
Data_Scale_X <- scale(Data_No_Scale_X, TRUE, TRUE)
Data <- cbind(Data_No_Scale_X,Data_No_Scale_y)
m <- nrow(Data)
n <- ncol(Data)
...
ggplot(data = Err_Data) +
  ...
  geom_point(aes(Min_Validation_Matrix_Col,as.numeric(Validation_error_Matrix[Min_Validation_Matrix_Col,1])))+
...
ggplot(data = as.data.frame(loss_Matrix)) +
  ...
  geom_point(aes(Min_Validation_Matrix_Col,as.numeric(Validation_MeanlogLoss_Matrix[Min_Validation_Matrix_Col,1])))+
  scale_colour_manual("",  breaks = c("validation", "train"),values = c("black", "red"))
